...
// link hackerank https://www.hackerrank.com/challenges/apple-and-orange/problem?isFullScreen=true&
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int apple_in_region = 0;
    int orange_in_region = 0;
    for (auto apple: apples) {
        int apple_final_pos = apple + a;
        if (apple_final_pos >= s and apple_final_pos <= t){
            apple_in_region = apple_in_region + 1;
        }
    }
    for (auto orange: oranges){
        int orange_final_pos = orange + b;
        if (orange_final_pos >= s and orange_final_pos <= t){
            orange_in_region = orange_in_region + 1;
        }
    }
    cout << apple_in_region << endl;
    cout << orange_in_region << endl;
